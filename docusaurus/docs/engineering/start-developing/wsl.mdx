...
:::warning
The following instructions are for Windows user only. If you use Linux or Mac and have already completed
the steps in [Setup Your Development Environment](/docs/engineering/start-developing/setting-up-dev-env), you
...
additional information or contextm or to ensure you have the most up-to-date
...
estimates appear as [<FontAwesomeIcon icon={faClock} /> time].
...
[<FontAwesomeIcon icon={faClock} /> 1 min] If you're using another Linux distro
you can add a second distro by
...
items that should be installed directly to Windows, while the Linux logo <FontAwesomeIcon icon={faLinux} />
indicates items that should be installed xdddsinto your chosen Linux distro (Ubuntu).
...
## <FontAwesomeIcon icon={faWindows} /> Installing Windows Terminal (Optional, but Recommended)
...
![terminal screenshot](/img/engineering/wsl/powershell_version.png)
...
[<FontAwesomeIcon icon={faClock} /> 2 min] to install the most recent version of
Powershell. Note that installing Powershell 7 _will not_ remove your Powershell
5 install. If you also have Powershell 5 installed, we recommend that you
manually uninstall it, as Windows occasionally gets confused concerning your
default terminal.

After that's complete,
[install Terminal](https://learn.microsoft.com/en-us/windows/terminal/install)
[<FontAwesomeIcon icon={faClock} /> 4 min] and follow the instructions to set
Powershell 7 as your default profile.

## <FontAwesomeIcon icon={faWindows} /> WSL Set-up

1. Complete the instructions in the
   [Install WSL](https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command)
   [<FontAwesomeIcon icon={faClock} /> 1 min] section.
...
2. Set-up your Linux username and password by
   ...
   [<FontAwesomeIcon icon={faClock} /> 2 min].

3. Follow the
   [instructions here](https://learn.microsoft.com/en-us/windows/wsl/install#upgrade-version-from-wsl-1-to-wsl-2)
   [<FontAwesomeIcon icon={faClock} /> 1 min] to verify you've installd WSL2 or
   to upgrade from WSL to WSL2, if needed.
...
2. If you have Windows Terminal open already, click the <FontAwesomeIcon icon={faChevronDown} />
to access the dropdown menu and display the profiles you can open. Then
select Ubuntu or your Linux distro from the list.

<!-- prettier-ignore-end -->

<img
  src="/img/engineering/wsl/powershell_profiles.png"
  style={{ width: "45%", paddingRight: 5 }}
/>
<img
  src="/img/engineering/wsl/powershell_wsl_tab.png"
  style={{ width: "45%" }}
/>
...
2. Verify your nvm insall by running:
...
3. Which version of Node is needed depends primarily on what you are going to be
   ...
   (BobaFrontend, BobaComponents, etc), use the below command to install
   NodeJS 16.
...
**You should install this if you plan working on BobaBackend, BobaFrontend, etc.
It is not needed for boba-docs! If you're confused about whether you need
...
- `mv [oldname] [newname]` renames the folder with `[oldname]` to `[newname]`
