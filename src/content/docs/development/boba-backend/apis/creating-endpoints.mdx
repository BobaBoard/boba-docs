...
You can learn more in our [intro to APIs](../intro).
...
endpoints at the `[server_address]/boards/*` URLs. In Express, the server
...
- A `tests/` folder, containing tests for the routes and queries. You can read
  ...
  [testing endpoints guide](../testing-endpoints).
...
2. Decide which HTTP Method (`GET`, `POST`, `PATCH`, etc.) your endpoint will
   use. The [REST API example](../intro#rest-api-example) includes examples of
...
{/* TODO: This section is incomplete. Ask Ms. Boba to fill it out! */}
